...
const Product = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setIsLoading(true);
      try {
        const response = await fetch('https://ggapi-production.up.railway.app/api/products');
        if (!response.ok) {
          throw new Error('Failed to fetch products');
        }
        const data = await response.json();
        // Check if data exists and has MyProducts array
        if (data && Array.isArray(data.MyProducts)) {
  ...
  return (
    <>
      <div className="font-poppins p-4">
        <h1 className="text-2xl font-semibold mb-4 pl-20">Our Products</h1>
      </div>
